...
import {useNavigate} from "react-router-dom"; // Keep navigate for internal navigation
              ...
              <div className="notification">
                  ...
                  {dropdownOpenNotifications && (
                      <div className="dropdownMenu notificationsDropDown" onMouseLeave={closeDropdownNotifications}>
                           {notifications.length === 0 && <span className="dropdownItem">No notifications</span>}
                          {/* Rendering the already sorted notifications */}
                          {notifications.map((notification: Notification) =>
                              <a key={notification.id} href={notification.navigationURL} className={notification.isRead ? "dropdownNotification" : "dropdownNotificationNew"}>
            ...
            {/* Profile Dropdown */}
             <li className="menuListItem"> {/* Wrap in li for structure */}
               <div className="profileWrapper">
                   {/* Use currentProfilePic state here */}
                  <img
                      src={currentProfilePic || defaultImage}
                      alt="Profile"
                      className="profileImage"
                      onClick={toggleDropdown}
                      onError={(e) => { // Add error handling for the image itself
                  ...
                  {dropdownOpen && (
                      <div className="dropdownMenu" onMouseLeave={closeDropdown}>
                        ...
                        {isMobile && <a href="/organizationList" className="dropdownItem">Browse Organizations</a>}
                          ...
                          <a className="dropdownItem" onClick={handleLogoutClick} style={{cursor: 'pointer'}}>Logout</a>
                      </div>
                  )}
              </div>
             </li>
